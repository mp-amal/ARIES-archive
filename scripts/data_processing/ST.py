...
# Kewords using for our archive standerds
...
telescope_df = pd.read_csv(paths_yaml['file_telescopes'], sep=',', comment='#').set_index('ShortName')    # DataFrame of telsecope details
instrument_df = pd.read_csv(paths_yaml['file_instruments'], sep=',', comment='#').set_index('ShortName')  # DataFrame of instrument details
keywords_df = pd.read_csv(paths_yaml['file_keywords'], sep='\s+', comment='#').set_index('Header')        # Details of MaseterHeder file
...
def move_fits_files(source_folder, destination_folder,Base_folder):
                ...
                with fits.open(filepath) as hdul:
                    ...
                    if naxis1 == 512 and naxis2 == 512:
                        ...
                        # If not, create the directory (and any intermediate directories if necessary)
                            ...
                            print(f"{destination_folder} created")
                        ...
                        after_8digits = re.split(r"\d{8}/", path, maxsplit=1)[1]
                        # print(after_8digits)
                        os.makedirs(os.path.dirname(os.path.join(Base_folder,after_8digits)),exist_ok=True)
                        shutil.copy(filepath, os.path.join(Base_folder,after_8digits))
                        print("imageing copied")
                        print( os.path.join(Base_folder,after_8digits))
                        # if not os.listdir(os.path.dirname(filepath)):
                        #     os.rmdir(os.path.dirname(filepath))
                        # print(destination_folder,filepath)
...
def extract_and_check_filter1(file_path,folder_name,file_template,f,Base_folder):
...
def datedisplay(source_directory, Base_folder,polarimetry):
    folder_list = sort_date(source_directory)
    process_list = sort_date(Base_folder)
    polarimetry_list = sort_date(polarimetry)
    print(folder_list,process_list,polarimetry_list)
    
    print("\n" * 3)
    folder_in_action =[]
    for folder in folder_list:
        if folder not in process_list and folder not in polarimetry_list:
            # print(folder)
            folder_in_action.append(folder)
            # process_fits_files(source_directory, folder, instrument_df)
    folders_to_copy= folder_in_action
    
    with open(paths_yaml['ST_fodlertocopy'], 'w') as f:
        f.write("Folder name\n")
        for folder_name in folder_in_action:
            if folder_name in folder_list:
                # f.write(f"{folder_name}\n")
                # copy_folder(source_directory, Base_folder, folder_name, paths_yaml['ST_fodlertocopy'])
                move_fits_files(os.path.join(source_directory,folder_name), os.path.join(polarimetry,folder_name),os.path.join(Base_folder,folder_name))
    ...
    #         move_fits_files(Base_folder+'/'+folder_name, desitnation_folder)
            file_template =paths_yaml['DIR_DOC']+f'{cycle}_temp/TemplateFile_ST_'+str(folder_name)+'.dat'

            if not os.path.exists(os.path.join(Base_folder,folder_name)):
                pass
            else:
                generate_template(os.path.join(Base_folder,folder_name), file_template)
                extract_and_check_filter1(file_template,folder_name,file_template,f,Base_folder)
...
def format_header(TELESCOPE, filename):
    ...
    header['DATE-OBS'] = date_obs.replace('"T"', 'T')
...
def do_astrometry(telescopename, filename, pixtolerance=0.02):
    ...
    try:
        ...
        try:
            ...
            subprocess.call('solve-field --continue --downsample 2 --no-plots --scale-low ' + PIX_l + ' --scale-high ' + PIX_u  + ' --scale-units app --config '+paths_yaml['DIR_DOC']+'/Astrometry.cfg --ra ' + str(LST_deg) + ' --dec ' + str(OBS_LAT) + ' --radius 100 ' + filename, timeout=40, shell=True)
...
def main():
    # date = '20260215'
    source_directory = os.path.join(paths_yaml['ST_path'],cycle,'rawdata')
    os.makedirs(source_directory,exist_ok=True)
    Base_folder = os.path.join(paths_yaml['ST_path'],cycle,"Processed_Data")
    os.makedirs(Base_folder,exist_ok=True)
    polarimetry_path = os.path.join(paths_yaml['ST_path'],cycle,'Polarimetry')

    os.makedirs(polarimetry_path,exist_ok=True)
    datedisplay(source_directory, Base_folder,polarimetry_path)
    process_folders =os.path.join(paths_yaml['DIR_DOC'],'ST_Process_folders.dat')
    process_file_df = pd.read_csv(process_folders, comment='#').set_index('Folder name')
    print("Processing path : ",process_file_df)
    for subfolder in process_file_df.index: 
        subfolder_path = os.path.join(Base_folder,str(subfolder))
        file_template = paths_yaml['DIR_DOC']+'/'+cycle+'_temp/TemplateFile_ST_'+str(subfolder)+'.dat'
...
#         # print(header_df)
    ...
    #                 # print(folder)
                ...
                if len(fits_files) !=0 :
                    ...
                    dest_path = f'/data/ST/{cycle}Final_data/'+str(year)+"/"+str(month)+"/"+str(telescope)+"/"+str(instrument)+"/"+str(date_folder)
