...
# Currently unused / commented imports
# import smtplib
# import calendar
# import schedule
...
# ============================================================
# Third-party astronomy and FITS/WCS tools
# ============================================================
import ephem
# import aplpy
...
# Duplicate imports kept intentionally
import matplotlib.pyplot as plt
import numpy as np
...
def datedisplay(source_directory, Base_folder):
    ...
    with open(paths_yaml['processing_folders'], 'w') as f:
        f.write("Folder name\n")
        for folder_name in folders_to_copy:
            if folder_name in folder_list:
                # f.write(f"{folder_name}\n")
                copy_folder(source_directory, Base_folder, folder_name, paths_yaml['processing_folders'])
                rewrite_file_name(Base_folder+'/'+folder_name)            
                file_template = paths_yaml['DIR_DOC']+'/'+cycle+'_temp/TemplateFile__'+str(folder_name)+'.dat'
    ...
    try:
        ...
        try:
            subprocess.call('solve-field --continue --downsample 2 --no-plots --scale-low ' + PIX_l + ' --scale-high '
                            + PIX_u + ' --scale-units app --config '+paths_yaml['DIR_DOC']+'/Astrometry.cfg --ra ' + str(LST_deg) + ' --dec ' +
...
source_directory =os.path.join(paths_yaml['DFOT_source_directory'],cycle,'rawdata')
Base_folder =os.path.join(paths_yaml['DFOT_Base_folder'],cycle,'Processed_data')

# print(source_directory)
def main():

    datedisplay(source_directory, Base_folder)
    process_folders =os.path.join(paths_yaml['DIR_DOC'],'Process_folders.dat')
    process_file_df = pd.read_csv(process_folders, comment='#').set_index('Folder name')
    print(process_file_df)
    for subfolder in process_file_df.index: 
        subfolder_path = os.path.join(Base_folder,str(subfolder))
        file_template = os.path.join(paths_yaml['DIR_DOC'],cycle+'_temp/TemplateFile__'+str(subfolder)+'.dat')
